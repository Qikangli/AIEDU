...
传统CNN和FCN结构上、功能上的区别
全连接层和卷积层如何相互转化
为什么传统CNN分类网络的输入图片是固定大小？
把全连接层的权重W重塑成卷积层的滤波器有什么好处：输入、输出、参数数量
FCN-8s/16s的结构和功能，分割细节缺失的缺点
Unet的结构、各部分功能尤其是级联
空洞卷积的作用、操作方法；普通卷积和空洞卷积的feature map、感受野的计算
segnet、RefineNet、densenet？？？==
...
==传统CNN分类和图像分割的区别：一个是一张图片分类，一个是每个像素都要分类。
传统CNN和FCN的区别
1、传统CNN的话，结构是多个卷积层后面接全连接softmax，输入一张图像，对整张图像进行一个分类(如VGG和RESnet)；
2、fcn的话，结构里全是卷积层(卷积+反卷积)+softmax，输入整张图像，提取抽象语义特征(图像级别的分类)，然后再上采样恢复图像大小，完成每个像素的分类==
...
==如何将卷积层转换成全连接层：feature map展平，每个元素就是一个神经元的输出；一个神经元只有几个w不是0、其余为0；属于同一个feature map的神经元之间w相同==
&emsp;&emsp;
...
==全连接层转为卷积层：卷积kernel大小设置与输入size一致，一个kernel对应一个全连接层神经元。==
&emsp;&emsp;
...
==全连接层转为卷积层的作用：可以减少w参数；如对于单个神经元，输入7x7x512的话，全连接层需要7x7x512个参数，而卷积层由于通道维度的参数共享，只需要7x7个参数。==
&emsp;&emsp;
...
==卷积层和池化层无所谓输入的大小；但全连接层参数数量固定，所以输入的大小固定，层层递进，回到第一层，也必须固定的。==
...
&emsp;&emsp;
==好处：1、输入大小不固定：首先不需要固定输入(待分类图像)的大小了，输出的是每一个局部区域的类别概率，假如图片不太大的话，取平均得到整体类别；
我觉得一般不会这么用，还是固定输入大小；只是相对于全连接层来说，确实更灵活。
2、输出多个区域类别：传统CNN需要做多次前向计算；而FCN只需一次前向计算就可以输出多个局部区域的类别概率(如识别图中很多数字，将图放大，输出所有局部区域的数字类别概率)==。  
&emsp;&emsp;
...
&emsp;&emsp;&emsp;&emsp;
1) 升采样分为两次完成（橙色×2）, 在第二次升采样前，把第4个pooling层（绿色）的预测结果（蓝色）融合进来。使用跳级结构提升精确性。
...
==他们的区别是一次、两次、三次上采样，步长不同，且有做特征融合==
...
### 9.2.10 FCN缺点
&emsp;&emsp;
（1）得到的结果还是不够精细。进行8倍上采样虽然比32倍的效果好了很多，但是上采样的结果还是比较模糊和平滑，对图像中的==细节不敏感（图像细节遗失）==。  
&emsp;&emsp;
...
&emsp;&emsp;
==结构的关键点：卷积、下采样、转置卷积(进行上采样)、级联、1x1卷积通道压缩(得到类别得分)、softmax==。
...
==级联的作用是信息补充：将低阶纹理特征和高阶抽象特征联合在一起，补充由于下采样导致的空间信息丢失，由后续的卷积层进行高低阶特征整合==  
&emsp;&emsp;
(4)	最后再经过两次反卷积操作，生成特征图，再用两个1X1的卷积做分类得到最后的==两张heatmap==,例如第一张表示的是第一类的==得分==，第二张表示第二类的得分heatmap,然后作为==softmax==函数的输入，算出概率比较大的softmax类，选择它作为输入给交叉熵进行反向传播训练。
...
&emsp;&emsp;
在图像分割领域，图像输入到CNN（典型的网络比如FCN[3]）中，FCN先像传统的CNN那样对图像做卷积再pooling，降低图像尺寸的同时增大感受野，但是由于图像分割预测是pixel-wise的输出，所以要将pooling后较小的图像尺寸upsampling到原始的图像尺寸进行预测（upsampling一般采用deconv反卷积操作，deconv可参见知乎答案如何理解深度学习中的deconvolution networks？），之前的pooling操作使得每个pixel预测都能看到较大感受野信息。因此图像分割FCN中有两个关键，一个是pooling减小图像尺寸增大感受野，另一个是upsampling扩大图像尺寸。在先减小再增大尺寸的过程中，肯定有一些信息损失掉了，那么能不能设计一种新的操作，==(作用)不通过pooling也能有较大的感受野看到更多的信息==呢？答案就是==dilated conv==。  
==它的作用是不通过pooling，增大感受野，减少空间结构信息的丢西==
&emsp;&emsp;
以前的CNN主要问题总结：   
&emsp;&emsp;
...
==总结：1、卷积的feature map大小运算：out=1+(in-kernelsize+2xpadding)/stride；对于空洞卷积，带入式中的kernelsize是卷积原本kernel每个点中间补充(dilated rate-1)的卷积
2、感受野的计算：(上层感受野-1)*stride+kernelsize==
